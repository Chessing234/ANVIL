# Demo video script — `splunk_agentic_ops`
...
**Screen:** Title slide with `TUTORIAL-Ops: Agentic Security Operations with a Real-Time STEM Education Pipeline` and tagline.  
...
integrations relevant to `splunk_agentic_ops`.
...
**Narration:** Tie each UI/API element to the hackathon angle: Splunk MCP integration, SPL generation with validation, threat-hunt templates, and live observability hooks that narrate operator actions into teachable moments.…
