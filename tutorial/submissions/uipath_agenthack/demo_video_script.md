# Demo video script — `uipath_agenthack`
...
**Screen:** Title slide with `TUTORIAL-Orchestra: Multi-Agent Orchestration for Autonomous Security & Education` and tagline.  
...
integrations relevant to `uipath_agenthack`.
...
**Narration:** Tie each UI/API element to the hackathon angle: UiPath Maestro-style queue orchestration, dynamic task routing, and human-in-the-loop handoffs modeled after enterprise RPA control rooms — submit to Track 1 (Maestro Case).…
