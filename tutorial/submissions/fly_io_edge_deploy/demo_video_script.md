# Demo video script — `fly_io_edge_deploy`
...
**Screen:** Title slide with `TUTORIAL-Fly: Global Edge Demos for Classroom-Scale SOC` and tagline.  
...
integrations relevant to `fly_io_edge_deploy`.
...
**Narration:** Tie each UI/API element to the hackathon angle: Single-command Docker Compose, health-checked API, and static SPA suitable for Fly Machines or any container edge with persistent volumes for SQLite workloads.…
